...
ARG JENA_VERSION
ARG FUSEKI_DIR
ARG FUSEKI_DATA_FILES_DIR
ARG FUSEKI_JAR
ARG JENA_ZIP
ARG REPO=https://repo1.maven.org/maven2/org/apache/jena
ARG JAR_URL=${REPO}/jena-fuseki-server/${JENA_VERSION}/${FUSEKI_JAR}
ARG ZIP_URL=${REPO}/apache-jena/${JENA_VERSION}/${JENA_ZIP}
...
#Download jena libs
RUN ./download.sh --chksum sha1 "$ZIP_URL"

RUN unzip $JENA_ZIP

#create data Directory
RUN mkdir $FUSEKI_DATA_FILES_DIR
...
ENV \
    ...
    FUSEKI_DATA_FILES_DIR=${FUSEKI_DATA_FILES_DIR}
